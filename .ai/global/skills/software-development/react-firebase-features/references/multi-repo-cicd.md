# Multi-Repo CI/CD Pattern — Session 2026-08-20

## Context

When the user asked for security scanning + PR review as GitHub Actions across all repos (MiNegocio, nas-gateway, nas-services, nas-infrastructure, AIEP), the solution was a single reusable workflow file copied to every repo.
...
1. **security-scan** — gitleaks (secrets), CodeQL (SAST), dependency audit (npm/pip)
...
- **Conditional execution** — jobs detect relevant files (`hashFiles('package.json') != ''`) to skip inapplicable repos
