# Security Audit: Web-Stack Patterns (Angular + Supabase + Nginx + Docker)

## Why This Reference Exists

The base `security-audit` skill is Python-centric. Web projects (Angular/Ionic + Supabase + Docker) require different patterns that the base skill doesn't cover. This reference fills that gap.

## Static Scan for Web Projects

```bash
# Docker Compose — secrets with default values (CRITICAL)
...
# Docker Compose — PostgreSQL exposed publicly (should be localhost-only)
grep -rn "5432:5432" docker-compose.yml | grep -v "127.0.0.1"

# Angular — console.log in production (use no-console ESLint rule)
...
# Docker Compose — healthcheck presence
